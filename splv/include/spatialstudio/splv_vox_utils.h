 ...
 * TODO: triple pointer :(
 */
SPLV_API SPLVerror splv_vox_load(const char* path, SPLVframe*** outFrames, uint32_t* numOutFrames, SPLVboundingBox* bbox);

/**
 * destroys all frames allocated from splv_vox_load()
 */
SPLV_API void splv_vox_frames_destroy(SPLVframe** frames, uint32_t numFrames);
 ...
 */
SPLV_API SPLVerror splv_vox_get_max_dimensions(const char* path, uint32_t* width, uint32_t* height, uint32_t* depth);
